...
(function(nfSearcher){
    ...
    _nfSearcher.prototype = {
        register: function() {
            this.$targetInput.keyup(e => {
                console.log("test");
                if ([13, 16, 20, 37, 38, 39, 40, 91].indexOf(e.width) === -1) {
        ...
        search: function() {
            let text = this.$targetInput.val();
            if (!text) {
                return;
            }

            if (this.shouldRedirect || !this.$groups) {
                this.redirect(text);
                return;
            }
        ...
        loadJsonAndSearch: function(text) {
            $.getJSON( "/src/json/search.json", data => {
                this.postList = data;
                this.doSearch(text);
            });
        },
        ...
        callback: function(matches) {
            ...
            for (let i = 0; i < matches.length; i++) {
                let post = matches[i];
                let group = document.getElementById(post.category);
