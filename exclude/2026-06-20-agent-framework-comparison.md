---
layout: post
title: "AI Agent 框架全景对比：从横向特征到分层架构"
date: 2026-06-20
categories:
  - Tech
tags:
...
从下到上：离应用程序越来越远，系统级能力越来越强；Runtime 能力越来越强；企业级能力越来越完整。
...
| Claude Agent SDK | System-level Agent Runtime | 如何构建具备文件系统、Shell、MCP 能力的 Agent |
...
执行模型：Agent A → handoff → Agent B → Tool → Guardrails

> 快速构建生产级 Multi-Agent 应用。

优势：Multi-Agent Handoff、Tracing、Guardrails
不足：Runtime 能力有限，不擅长 Coding Agent 场景

### Claude Agent SDK — System Runtime Layer

来源：Claude Code Runtime 能力开放。

- Filesystem Native / Bash Native / MCP Native
- Subagents / Hooks / Session Persistence

执行模型：Agent ↔ Filesystem ↔ Bash ↔ MCP Tools ↔ Subagents

> Agent 是一个可操作真实系统环境的进程。

优势：Coding Agent 天然适配、MCP 生态成熟、文件系统能力强、Shell 执行能力强
...
执行模型：Event → Runner → Agents → Tools/Subagents → Memory → Artifacts → Event

> Agent 是一个可观测、可部署、可运维的系统。

优势：Runtime 完整、多 Agent 原生支持、生命周期管理、企业级能力

---

## 四、DeepAgents vs OpenCode
...
这是几个框架最本质的区别——它们对"Agent 所处的世界"有不同的假设。
...
| 系统 | Tool 体系 |
|--------|--------|
| DeepAgents | Function Calling + 内置编程工具集（文件读写、TODO 规划、子任务派发） |
...
| 系统 | 哲学 | 优先级 |
|--------|------|--------|
| DeepAgents | Make agents easy. | 开发体验优先 |
| OpenAI Agents SDK | Make multi-agent apps production-ready. | 应用开发优先 |
| Claude Agent SDK | Make agents behave like real systems. | 系统执行能力优先 |
| ADK | Make agents enterprise-scale systems. | 平台与运维能力优先 |
...
|--------|--------|
| LangGraph | 流程引擎 |
| DeepAgents | Agent Harness，轻、可读、模型无关，适合实验性/自定义项目 |
| OpenCode | 终端可直接用的产品，多 provider 是亮点，subagent 偏角色化分工 |
| OpenAI Agents SDK | Multi-Agent Application Runtime，四原语清晰、Handoff 范式独特、Guardrail 和 Tracing 开箱即用 |
| Claude Agent SDK | System-level Agent Runtime，subagent 隔离最彻底但只能两层，工具按需加载是独门武器 |
| ADK | Agent Platform OS，企业全生命周期框架，概念最全最重，功能最完整 |
